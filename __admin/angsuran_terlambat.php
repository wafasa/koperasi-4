...
$isi.="<div class=kepala>Data Transaksi &raquo; <a href='angsuran_terlambat.php'>Laporan Angsuran Terlambat</a></div>";
...
";
	...
	$tgl = date("Y-m-d");
	$sql = mysql_query("select n.rek_pinjaman, n.nama, p.*, a.* from tb_nasabah n, tb_pinjaman p, tb_angsuran a where p.no_anggota = n.no_anggota and p.no_pinjaman = a.no_pinjaman and a.status_lunas = '0' and a.jatuh_tempo < '$tgl'",$conn);
	$data= mysql_fetch_array(mysql_query("select sum(pokok) as pokok, sum(jasa) as jasa, sum(denda) as denda from tb_angsuran where jatuh_tempo < '$tgl' and status_lunas = '0'",$conn));
	}
	...
	$isi.="</tbody>
	<tr><td colspan=6>JUMLAH</td><td>".duit($data[pokok])."</td><td>".duit($data[jasa])."</td></tr>
