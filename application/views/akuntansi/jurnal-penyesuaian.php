...
$(function() {
    ...
    $('#bt_cari').click(function() {
        ...
        $('#datamodal_no .title h4').html('Tambah Jurnal Penyesuaian');
...
function tampilkan(p, id_bukubesar) {
    ...
    $.ajax({
        ...
        data: $('#form_jurnal').serialize()+'&jenis=Penyesuaian',
        dataType: 'json',
...
//                        '<td>'+v.transaksi+'</td>'+
                        ...
                        str+='</td>'+
                        '<td align="right">'+
                            '<button type="button" class="btn btn-default btn-xs" onclick="edit_jurnal(\''+v.no_transaksi+'\')"><i class="fa fa-pencil"></i></button> '+
                            '<button type="button" class="btn btn-default btn-xs" onclick="delete_jurnal(\''+v.no_transaksi+'\','+p+')"><i class="fa fa-trash-o"></i></button>'+
                        '</td>'+
                    '</tr>';
...
function edit_jurnal(no_transaksi) {
    $('#datamodal_no').modal('show');
    $('#datamodal_no .title h4').html('Edit Jurnal Penyesuaian');
    $.ajax({
        type: 'GET',
...
function save_jurnal(){
    ...
    for (i = 0; i <= debet-1; i++) {
        if ($('#rekening'+i).val() === '') {
            dinamic_alert('Rekening masih ada yang kosong !'); return false;
        }
        if ($('#nilai'+i).val() === '') {
            dinamic_alert('Nominal masih ada yang kosong !'); return false;
        }
    }
    ...
    if ($('#total_debet').html() !== $('#total_kredit').html()) {
        dinamic_alert('Total debet dan kredit belum sesuai'); return false;
    }
    ...
    $.ajax({
        type : 'POST',
        url: '<?= base_url("api/akuntansi/jurnal") ?>',
        data: $('#form').serialize()+'&jenis=Penyesuaian',
        cache: false,
        dataType : 'json',
        success: function(data) {
...
<div id="datamodal_jurnal" class="modal fade">
    ...
    <div class="modal-content">
        ...
        <div class="modal-footer">
            <button type="button" class="btn btn-default" data-dismiss="modal"><i class="fa fa-refresh"></i> Close</button>
            <button type="button" class="btn btn-primary" onclick="tampilkan(1);"><i class="fa fa-save"></i> Tampilkan</button>
        </div>
    </div>
    </div>
</div>
