...
<script type="text/javascript">
    $(function() {
        get_list_kategori_anggota(1);
        $('#add_kategori_anggota').click(function() {
            reset_form();
        ...
        $('#reload_kategori_anggota').click(function() {
            reset_form();
            get_list_kategori_anggota(1);
        });
    ...
    function get_list_kategori_anggota(p, id) {
        $('#datamodal_search').modal('hide');
        var id = '';
        $.ajax({
            type : 'GET',
            url: '<?= base_url("api/masterdata/kategori_anggotas") ?>/page/'+p+'/id/'+id,
                ...
                $.each(data.data,function(i, v){
                    ...
                    str+= '<tr data-tt-id='+i+' class="'+highlight+'">'+
                            ...
                            '<td align="right" class=aksi>'+
                                '<button type="button" class="btn btn-default btn-mini" onclick="edit_kategori_anggota(\''+v.id+'\')"><i class="fa fa-pencil"></i></button> '+
                                '<button type="button" class="btn btn-default btn-mini" onclick="delete_kategori_anggota(\''+v.id+'\','+data.page+');"><i class="fa fa-trash-o"></i></button>'+
                            '</td>'+
                        '</tr>';
    ...
    function edit_kategori_anggota(id) {
        
        $.ajax({
            type: 'GET',
            url: '<?= base_url('api/masterdata/kategori_anggotas') ?>/page/1/id/'+id,
            dataType: 'json',
            success: function(data) {
                $('#datamodal_add').modal('show');
    ...
    function save_kategori_anggota() {
        $.ajax({
            type: 'POST',
            url: '<?= base_url('api/masterdata/kategori_anggota') ?>',
            ...
            success: function(msg) {
                ...
                if (msg.act === 'add') {
                    ...
                    message_add_success();
                    get_list_kategori_anggota(1, msg.id);
                } else {
                    $('#datamodal_add').modal('hide');
                    message_edit_success();
                    get_list_kategori_anggota(page);
                }
            },
            error: function() {
    ...
    function delete_kategori_anggota(id, page) {
        bootbox.dialog({
            ...
            buttons: {
              ...
              ya: {
                ...
                callback: function() {
                    $.ajax({
                        type: 'DELETE',
                        url: '<?= base_url('api/masterdata/kategori_anggota') ?>/id/'+id,
                        dataType: 'json',
                        success: function(data) {
      ...
      <div class="row">
          ...
          <div class="grid simple ">
            <div class="grid-title">
              <h4>Daftar List <?= $title ?></h4>
                <div class="tools"> 
                    <button id="add_kategori_anggota" class="btn btn-info btn-mini"><i class="fa fa-plus-circle"></i> Tambah Data</button>
                    <button id="cari_button" class="btn btn-mini"><i class="fa fa-search"></i> Cari</button>
                    <button id="reload_kategori_anggota" class="btn btn-mini"><i class="fa fa-refresh"></i> Reload Data</button>
                </div>
            </div>
            <div class="grid-body">
        ...
        <div id="datamodal_search" class="modal fade">
          ...
          <div class="modal-content">
            ...
            <div class="modal-footer">
              <button type="button" class="btn btn-default" data-dismiss="modal"><i class="fa fa-refresh"></i> Batal</button>
              <button type="button" class="btn btn-primary" onclick="get_list_kategori_anggota(1);"><i class="fa fa-eye"></i> Tampilkan</button>
            </div>
          </div><!-- /.modal-content -->
        </div><!-- /.modal-dialog -->
        </div><!-- /.modal -->
