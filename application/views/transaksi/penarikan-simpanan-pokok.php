...
<script type="text/javascript">
    $(function() {
        ...
        get_list_penarikan_simpanan_pokok(1);
        $('#add_penarikan_simpanan_pokok').click(function() {
            reset_form();
        ...
        $('#reload_penarikan_simpanan_pokok').click(function() {
            reset_form();
            get_list_penarikan_simpanan_pokok(1);
        });
        ...
        $('#norek').select2({
            ...
            formatSelection: function(data){
                ...
                get_saldo_penarikan_simpanan_pokok(data.id);
    ...
    function get_saldo_penarikan_simpanan_pokok(id_anggota) {
        $.ajax({
            type: 'GET',
            url: '<?= base_url('api/transaksi/saldo_simpanan_pokok') ?>/page/1/id/'+id_anggota,
    ...
    function get_list_penarikan_simpanan_pokok(p, id) {
        $('#datamodal_search').modal('hide');
        var id = '';
        $.ajax({
            type : 'GET',
            url: '<?= base_url("api/transaksi/penarikan_simpanan_pokoks") ?>/page/'+p+'/id/'+id,
            ...
            success: function(data) {
                if ((p > 1) & (data.data.length === 0)) {
                    get_list_penarikan_simpanan_pokok(p-1);
                    return false;
                };
                ...
                $.each(data.data,function(i, v){
                    ...
                    str+= '<tr data-tt-id='+i+' class="'+highlight+'">'+
                            ...
                            '<td align="right" class=aksi>'+
                                ...
                                '<button type="button" class="btn btn-default btn-mini" onclick="cetak_penarikan_simpanan_pokok(\''+v.id+'\');"><i class="fa fa-print"></i></button> '+
                                //'<button type="button" class="btn btn-default btn-mini" onclick="edit_penarikan_simpanan_pokok(\''+v.id+'\')"><i class="fa fa-pencil"></i></button> '+
                                '<button type="button" class="btn btn-default btn-mini" onclick="delete_penarikan_simpanan_pokok(\''+v.id+'\','+data.page+');"><i class="fa fa-trash-o"></i></button>'+
                            '</td>'+
                        '</tr>';
    ...
    function cetak_penarikan_simpanan_pokok(id) {
    ...
    function edit_penarikan_simpanan_pokok(id) {
        $('#oldpict').html('');
        $('#datamodal').modal('show');
        $('#datamodal h4.modal-title').html('Edit <?= $title ?>');
        $.ajax({
            type: 'GET',
            url: '<?= base_url('api/transaksi/penarikan_simpanan_pokoks') ?>/page/1/id/'+id,
            dataType: 'json',
    ...
    function paging(p) {
        get_list_penarikan_simpanan_pokok(p);
    }
    ...
    function konfirmasi_save() {
        ...
        if (parseFloat(nominal) > parseFloat(saldo)) {
            dc_validation('#nominal_tabungan','Nominal pengambilan tidak boleh lebih besar dari saldo !'); return false;
        }
        bootbox.dialog({
    ...
    function save_penarikan_simpanan_pokok() {
        $.ajax({
            type: 'POST',
            url: '<?= base_url('api/transaksi/penarikan_simpanan_pokok') ?>',
            dataType: 'json',
            ...
            success: function(msg) {
                ...
                if (msg.act === 'add') {
                    ...
                    get_list_penarikan_simpanan_pokok(page);
                }
            },
            error: function() {
                $('#datamodal').modal('hide');
    ...
    function delete_penarikan_simpanan_pokok(id, page) {
        bootbox.dialog({
            ...
            buttons: {
              ...
              ya: {
                ...
                callback: function() {
                    $.ajax({
                        type: 'DELETE',
                        url: '<?= base_url('api/transaksi/penarikan_simpanan_pokok') ?>/id/'+id,
                        dataType: 'json',
                        success: function(data) {
      ...
      <div class="row">
          ...
          <div class="grid simple ">
            <div class="grid-title">
              <h4>Daftar List <?= $title ?></h4>
                <div class="tools"> 
                    <button id="add_penarikan_simpanan_pokok" class="btn btn-info btn-mini"><i class="fa fa-plus-circle"></i> Tambah Data</button>
                    <button id="cari_button" class="btn btn-mini"><i class="fa fa-search"></i> Cari</button>
                    <button id="reload_penarikan_simpanan_pokok" class="btn btn-mini"><i class="fa fa-refresh"></i> Reload Data</button>
                </div>
            </div>
            <div class="grid-body">
        ...
        <div id="datamodal_search" class="modal fade">
          ...
          <div class="modal-content">
            ...
            <div class="modal-footer">
              <button type="button" class="btn btn-default" data-dismiss="modal"><i class="fa fa-refresh"></i> Batal</button>
              <button type="button" class="btn btn-primary" onclick="get_list_penarikan_simpanan_pokok(1);"><i class="fa fa-eye"></i> Tampilkan</button>
            </div>
          </div><!-- /.modal-content -->
        </div><!-- /.modal-dialog -->
        </div><!-- /.modal -->
