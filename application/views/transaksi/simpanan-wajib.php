...
<script type="text/javascript">
    $(function() {
        ...
        get_list_simpanan_wajib(1);
        $('#add_simpanan_wajib').click(function() {
            reset_form();
        ...
        $('#reload_simpanan_wajib').click(function() {
            reset_form();
            get_list_simpanan_wajib(1);
        });
        ...
        $('#norek').select2({
            ...
            formatSelection: function(data){
                ...
                get_saldo_simpanan_wajib(data.id);
    ...
    function get_saldo_simpanan_wajib(id_anggota) {
        $.ajax({
            type: 'GET',
    ...
    function get_list_simpanan_wajib(p, id) {
        $('#datamodal_search').modal('hide');
        var id = '';
        $.ajax({
            type : 'GET',
            url: '<?= base_url("api/transaksi/simpanan_wajibs") ?>/page/'+p+'/id/'+id,
                ...
                $.each(data.data,function(i, v){
                    ...
                    str+= '<tr data-tt-id='+i+' class="'+highlight+'">'+
                            ...
                            '<td align="right" class=aksi>'+
                                ...
                                '<button type="button" class="btn btn-default btn-mini" onclick="cetak_simpanan_wajib(\''+v.id+'\');"><i class="fa fa-print"></i></button> '+
                                '<button type="button" class="btn btn-default btn-mini" onclick="edit_simpanan_wajib(\''+v.id+'\')"><i class="fa fa-pencil"></i></button> '+
                                '<button type="button" class="btn btn-default btn-mini" onclick="delete_simpanan_wajib(\''+v.id+'\','+data.page+');"><i class="fa fa-trash-o"></i></button>'+
                            '</td>'+
                        '</tr>';
    ...
    function cetak_simpanan_wajib(id) {
        var wWidth = $(window).width();
        var dWidth = wWidth * 1;
        var wHeight= $(window).height();
        var dHeight= wHeight * 1;
        var x = screen.width/2 - dWidth/2;
        var y = screen.height/2 - dHeight/2;
        window.open('<?= base_url('transaksi/print_simpanan_wajib') ?>?id='+id,'Cetak Transaksi Pajak','width='+dWidth+', height='+dHeight+', left='+x+',top='+y);
    }
    ...
    function edit_simpanan_wajib(id) {
        ...
        $.ajax({
            type: 'GET',
            url: '<?= base_url('api/transaksi/simpanan_wajibs') ?>/page/1/id/'+id,
            dataType: 'json',
    ...
    function save_simpanan_wajib() {
        $.ajax({
            type: 'POST',
            url: '<?= base_url('api/transaksi/simpanan_wajib') ?>',
            dataType: 'json',
    ...
    function delete_simpanan_wajib(id, page) {
        bootbox.dialog({
            ...
            buttons: {
              ...
              ya: {
                ...
                callback: function() {
                    $.ajax({
                        type: 'DELETE',
                        url: '<?= base_url('api/transaksi/simpanan_wajib') ?>/id/'+id,
                        dataType: 'json',
                        success: function(data) {
      ...
      <div class="row">
          ...
          <div class="grid simple ">
            <div class="grid-title">
              <h4>Daftar List <?= $title ?></h4>
                <div class="tools"> 
                    <button id="add_simpanan_wajib" class="btn btn-info btn-mini"><i class="fa fa-plus-circle"></i> Tambah Data</button>
                    <button id="cari_button" class="btn btn-mini"><i class="fa fa-search"></i> Cari</button>
                    <button id="reload_simpanan_wajib" class="btn btn-mini"><i class="fa fa-refresh"></i> Reload Data</button>
                </div>
            </div>
            <div class="grid-body">
        ...
        <div id="datamodal_search" class="modal fade">
          ...
          <div class="modal-content">
            ...
            <div class="modal-footer">
              <button type="button" class="btn btn-default" data-dismiss="modal"><i class="fa fa-refresh"></i> Batal</button>
              <button type="button" class="btn btn-primary" onclick="get_list_simpanan_wajib(1);"><i class="fa fa-eye"></i> Tampilkan</button>
            </div>
          </div><!-- /.modal-content -->
        </div><!-- /.modal-dialog -->
        </div><!-- /.modal -->
