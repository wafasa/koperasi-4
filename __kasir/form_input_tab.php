...
$isi.="<div class=kepala>Transaksi &raquo; <a href='form_input_tab.php'>Form Input Tabungan</a></div>";
...
if (isset($_POST['cari_rek'])) {
	...
	if ($jum == '0') {
		...
		$duit = mysql_fetch_array($tab);
		$duit = duit($duit[saldo]);
	}
	...
	if ((trim($_POST['jumlah']) == '') or (trim($_POST['nama']) == '')) {
		...
		$jml = mysql_num_rows(mysql_query("select * from tb_tabungan where no_agt_tab = '$_POST[no_agt]'",$conn));
		if ($jml == '0') {
			new_tab($_POST[no_agt],1,$_POST[jumlah]);
		} else {
			add_tab($_POST[no_agt],$_POST[jumlah]);
		}
...
/*else {
	...
	$tab  = mysql_query("select saldo from tb_tabungan where no_agt_tab = '$data[no_anggota]'",$conn);
	$jum  = mysql_num_rows($tab);
	$no_rek = $_GET[no_rek];
	if ($jum == '0') {
		$duit = 0;
...
$isi.="
	...
	<tr><td colspan=4 class=judulform>Form Input Penabungan </td></tr>
	<tr>
	<td width=18% height=35 valign=middle class=CellLabel>Nomor Rekening Tabungan</td>
	<td height=35 class=cellIsian><form action='' method=post><input name='no_rek' type=text value='$no_rek' size=45 maxlength=50 /> &nbsp; <input type=submit class=tmbl name='cari_rek' value=' Go '></form></td>
	<td height=25 valign=middle class=CellLabel>Saldo Terakhir (Rp. )</td>
	<td height=25 valign=middle class=cellIsian><input name=saldo_akhir type=text value='$duit' size=50 maxlength=46 onKeyup=FormNum(this); disabled/></td></tr>
	</tr>
	<tr>
	...
	<td height=25 valign=middle class=CellLabel>Jumlah Nominal Penabungan &nbsp;(Rp. )</td>
