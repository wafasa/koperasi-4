...
";
	...
	while ($bar = mysql_fetch_array($sql)) {
		...
		<td align=center><a href='anggota_form_edit.php?id=$bar[no_anggota]&kd_atb=$bar[kd_atribut]' onmouseover= \"Tip('Edit Data Nasabah') \" onmouseout= \"UnTip()\"><img src='../images/b_edit.png'></a> 
		<a onclick=	\"ok=confirm('Anda yakin akan menghapus data anggota ".strtoupper($bar[nama])."?');
		if (ok) {
		return } else {return false} \" href='list_anggota.php?aksi=delete&id=$bar[no_anggota]' onmouseover= \"Tip('Hapus Data Nasabah') \" onmouseout= \"UnTip()\"><img src='../images/b_drop.png'></a></td>
		</tr>
		";
	$no +=1;
	}
